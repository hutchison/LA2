...
%
%\begin{mysatz} \label{CSU} \textit{Cauchy-Schwarzsche-Ungleichung} \par
%        Sei $V$ ein euklidischer Vektorraum. Dann gilt
%        \begin{eqnarray*}
%                        | \left\langle v,w \right\rangle | & \leq & \| v \| \cdot \| w \| \mbox{ oder in äquvalenter Weise}\\
%                        \left\langle v,w \right\rangle^2 & \leq & \left\langle v,v \right\rangle \cdot \left\langle w,w \right\rangle \qquad \forall v,w\in V
%        \end{eqnarray*}
%\end{mysatz}
%
%\begin{bew}
%        Wenn $w=0$ oder $v=0$, dann ist die Aussage trivial. \par
%        Daher sei $v,w\neq 0$. Dann gilt für jedes $k\in \R$ 
%                        \begin{equation*}
%                                        0 \leq \left\langle v-kw,v-kw \right\rangle = \left\langle v,v \right\rangle +k^2\left\langle w,w \right\rangle -2k \left\langle v,w \right\rangle
%                        \end{equation*}
%        Also insbesondere auch für $k=\frac{\left\langle v,w \right\rangle}{\left\langle w,w \right\rangle}$. Dann ist nämlich
%                        \begin{equation*}
%                                        0 \leq \left\langle v,v \right\rangle +\frac{\left\langle v,w \right\rangle^2}{\left\langle w,w \right\rangle^2}\left\langle w,w \right\rangle -2\frac{\left\langle v,w \right\rangle}{\left\langle w,w \right\rangle} \left\langle v,w \right\rangle
...
%
%\begin{bsp} \qquad \par
%        Im $\R^2,\R^3$ mit dem Standardskalarprodukt entspricht $\| v \|$ der Länge und $\varphi$ dem Winkel zwischen den Vektoren $v,w$ im 
%        elementargeometrischen Sinn.\par
%        
%        \begin{center}
%                \begin{tikzpicture}
%                        \draw[->] (0,0) -- (8.1,0);             % Vektor v
%                                        \draw (6,-0.2) node{$v$};
%                        \draw[->] (0,0) -- (4.5,2.625); % Vektor w
%                                        \draw (2.5,1.7) node {$w$};     
%                        \draw (0,0) -- (6,3.5);                                 % Strecke q     
%                        \draw (4.5,2.625) -- (4.5,0);
%                                        \draw (4.5cm,0.5cm) arc (90:180:0.5cm);
%                                        \draw (4.3cm,0.2cm) node {$\cdot$};
%                        \draw (6cm,3.5) -- (8.1cm,0);
%                                        \draw (6.25cm,3.05cm) arc(300:210:0.5);
%                                        \draw (5.95cm,3.2cm) node {$\cdot$};
%                        \draw (1cm,0) arc (0:30:1cm);   % Winkel alpha
%                                        \draw (0.6cm,0.15cm) -- (1.2cm,0.4cm);
%                                        \draw (2 cm,0.5cm) node {$\sphericalangle (v,w)$};
%                \end{tikzpicture}
%        \end{center}
%        
%        
%        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%        %%% Die Strecken p und q müssen noch beschriftet werden %%%
%        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%        \begin{eqnarray*}
%                \left.\begin{array}{c} \displaystyle{ \cos \sphericalangle (v,w) = \frac{p}{\| w \|} } \\ 
%                                                                                                         \displaystyle{ \cos \sphericalangle (v,w) = \frac{q}{\| v \|} }  \end{array}\right\}
%                                                                                                         &\Rightarrow& \left\langle v,w \right\rangle = q\cdot \| w \| = p\cdot \| v \| \qquad \mbox{ und damit } \\
...
%
%
%        Das Skalarprodukt von $v$ und $w$ ist die Länge von $v$ mal der Länge der Projektion von $w$ auf $v$. Desweiteren sind zwei Vektoren orthogonal 
%        zueinander, wenn das Skalarprodukt verschwindet. Dies gilt für $\varphi=\pi/2$ aber auch, wenn einer der beiden Vektoren der Nullvektor ist.
%\end{bsp}
%
%%---------------------------------------Definition 4.12-------------------------------------------
%%------------------------------------orthogonal,orthonormal---------------------------------------
%
%\begin{mydef} \label{orthogonal,orthonormal}     \textit{orthogonal, orthonormal} \par
%        Eine Menge $\{ v_1,\ldots,v_n \}$ von Vektoren in einem euklidischen Vektorraum $V$ heißt genau dann orthogonal, wenn 
%        $\left\langle v_i,v_j \right\rangle=0$ für alle $i=j\in \{ 1,\ldots,n \}$ und orthonormal, wenn $\left\langle v_i,v_j \right\rangle=\delta_{ij}$ ist. 
%\end{mydef}
%
%%------------------------------------------Lemma 4.13---------------------------------------------
%%-------------------------------------------------------------------------------------------------
%
%\begin{mylemma} \qquad \par
%        Eine orthogonale Menge, die den Nullvektor nicht enthält, ist linear unabhängig.
%\end{mylemma}
%\begin{bew}
%        Sei $\{ v_1,\ldots,v_n \}$ die orthogonale Menge. Die Vektoren $v_1,\ldots,v_n$ sind genau dann linear unabhängig, wenn aus 
%        $\sum_{i=1}^n k_i\cdot v_i = 0$ folgt, dass die Koeffizienten $k_i$ alle $0$ sind. Da die Vektoren orthogonal zueinander sind, folgt
%                \begin{equation*}
%                                0 = \left\langle \sum_{i=1}^n k_i\cdot v_i , v_j \right\rangle = k_j \cdot \left\langle \underbrace{v_j,v_j}_{\neq 0} \right\rangle
%                \end{equation*}
...
%
%\begin{mysatz} \textit{Orthonormalisierungsverfahren nach Eberhard Schmidt $(1876-1959)$} \par
%        Jeder endlich dimensionle euklische Vektorraum besitzt eine Orthonormalbasis, kurz ON-Basis.
%\end{mysatz}
%
%\begin{bew} \textit{Induktion über die Dimension $n$} \par
%        $n=1$: Sei $V=\left\langle b \right\rangle$. Dann ist $\{ e=\frac{b}{\| b \|}\}$ eine ON-Basis.\par \medskip
%        $n-1\rightarrow n$: Sei dazu $\{ e_1,\ldots,e_{n-1} \}$ eine ON-Basis von $U\subset V$. Dann kann man mit einem geeigenten $b_n\in V$ die 
%        Menge $\{ e_1,\ldots,e_{n-1},b_n \}$ zu einer Basis von $V$ ergänzen. Setze nun
%                        \begin{equation*}
%                                e_n' = \left(-\sum_{i=1}^{n-1} \frac{ \left\langle e_i,b_n \right\rangle }{ \left\langle e_i,e_i \right\rangle }\cdot e_i \right) + b_n
%                        \end{equation*}
%        Dann folgt für alle $k=1,\ldots,n-1$ 
%                        \begin{eqnarray*}
%                                \left\langle e_n',e_k \right\rangle & = & -\sum_{i=1}^{n-1} \frac{ \left\langle e_i,b_n \right\rangle }{ \left\langle e_i,e_i \right\rangle }\cdot \left\langle e_i,e_k \right\rangle + \left\langle b_n,e_k \right\rangle \\
%                                                                                                         & = & - \frac{ \left\langle e_k,b_n \right\rangle }{ \left\langle e_k,e_k \right\rangle } \cdot \left\langle e_k,e_k \right\rangle + \left\langle b_n,e_k \right\rangle \\
%                                                                                                         & = & 0
%                        \end{eqnarray*}
...
%        Sei 
%                \begin{equation*}
%                        \{ b_1,b_2,b_3 \} = \left\{ \begin{pmatrix} 1\\-1\\0\\1 \end{pmatrix},\begin{pmatrix} 1\\0\\1\\0 \end{pmatrix},\begin{pmatrix} 1\\-1\\0\\0 \end{pmatrix} \right\}
%                \end{equation*}
%        eine Basis von $U\subset \R^4$. Dann ist $e_1'=b_1$ und $e_2'=\lambda e_1'+b_2$, daher folgt
%                \begin{eqnarray*}
...
%        Also ist 
%                \begin{equation*}
%                                e_2' = -\frac{1}{3} \cdot \begin{pmatrix} 1\\-1\\0\\1 \end{pmatrix} + \begin{pmatrix} 1\\0\\1\\0 \end{pmatrix} = \frac{1}{3}\cdot \begin{pmatrix} 2\\1\\3\\-1 \end{pmatrix}.
%                \end{equation*}
%        Für den dritten Basisvektor muss gelten gilt $e_3'=\lambda_1 e_1 + \lambda_2 e_2 + b_3$. Daher folgt
%                \begin{eqnarray*}
...
%                \begin{equation*}
%                        e_3' = -\frac{2}{3}\cdot\begin{pmatrix}1\\-1\\0\\1\end{pmatrix}-\frac{1}{15}\cdot\begin{pmatrix} 2\\1\\3\\-1\end{pmatrix}+\begin{pmatrix}1\\-1\\0\\0\end{pmatrix}=\frac{1}{5}\begin{pmatrix}1\\-2\\-1\\-3 \end{pmatrix}
%                \end{equation*}
%        Normieren von der $e_i'$ liefert dann die ON-Basis
%        \begin{equation*}
%                        \left\{ \frac{1}{\sqrt{3}}\cdot \begin{pmatrix} 1\\-1\\0\\1 \end{pmatrix}, \frac{1}{\sqrt{15}}\cdot \begin{pmatrix} 2\\1\\3\\-1 \end{pmatrix}, 
%                                                                         \frac{1}{\sqrt{15}}\cdot \begin{pmatrix} 1\\-2\\-1\\3 \end{pmatrix} \right\}
%        \end{equation*}
%        
%        
%        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%        %  Beispiel nochma durchrechnen, irgendwas stimmt da nicht %
%        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\end{bsp}
%
%%----------------------------------------Bemerkung-------------------------------------------------
%
%\begin{bem} \qquad \par
%        Sei $\{ e_1,\ldots,e_n \}$ eine ON-Basis von $V$ und $v\in V$, dann sind die Koordinaten $k_i$ aus $\displaystyle{v=\sum k_i e_i}$ gegeben 
%        durch
%                \begin{equation*}
...
%
%\begin{mydef} \textit{Isometrische Abbildungen} \par
%        Eine lineare Abbildung $f$ eines euklidischen VR $V$ in einen euklidischen Vektorraum $W$ heißt genau dann isometrisch, wenn
%                \begin{equation*}
%                                \left\langle v,w \right\rangle_V = \left\langle f(v),f(w)\right\rangle_W \ \forall v,w \in W
%                \end{equation*}
%\end{mydef}
%
%%----------------------------------------Bemerkung-------------------------------------------------
%
%\begin{bem} \qquad \par
%        Seien $V$ und $W$ euklidische Vektorräume. Die lineare Abbildung $f:V\rightarrow W$ ist genau dann eine isometrische Abbildung, wenn sie 
%        invariant bzgl. der Normen der jeweiligen Vektorräume ist.
%                \begin{equation*}
...
%
%\begin{mylemma}     \qquad \par
%        Isometrische Abbildungen sind injektiv.
%\end{mylemma}
%
%\begin{bew}
...
%
%\begin{mylemma}     \qquad \par
%        Sei $\{ b_1,\ldots,b_n \}$ eine ON-Basis von $V$. $f$ ist genau dann isometrisch, wenn        $\{ f(b_1),\ldots,f(b_n) \}$ eine 
%        ON-Basis ist.
%\end{mylemma}
%
%\begin{bew}
%        $\Rightarrow$ Wenn $f$ isometrisch ist, dann folgt $\left\langle f(b_i),f(b_j) \right\rangle= \left\langle b_i,b_j \right\rangle=\delta_{ij}$.\par
%        $\Leftarrow$ Zu zeigen ist, dass $f$ isometrisch ist, wenn $\{ f(b_1),\ldots,f(b_n) \}$ eine ON-Basis ist. Seien dazu 
%        $v_1=\sum k_i\cdot b_i$ und $v_2=\sum l_i \cdot b_i$. Dann ist 
%        \begin{equation*}
%                \left\langle v_1,v_2 \right\rangle=\left\langle \sum_{i=1}^n k_i f(b_i), \sum_{i=1}^n l_i f(b_i)\right\rangle = \sum k_i\cdot l_i = \left\langle v_1,v_2 \right\rangle
%        \end{equation*}
%\end{bew} \par \medskip
%
%Im folgenden sei stets $V=W$. Dann wird $f$ zu einem isometrischen Endomorphismus (auch Automorphismus genannt). Diese Abbildungen bekommen einen bestimmten Namen. Sie heißen orthogonale Abbildungen. \par \medskip
%Die Menge aller orthogonalen Abbildungen ist eine Gruppe
%        \begin{equation*}
%                \mathcal{O}(n)\leq GL(n,\R)=\{ \mbox{reguläre Matrizen vom Typ $n\times n$} \}
%        \end{equation*}
%
%%----------------------------------------Bemerkung-------------------------------------------------
%
%\begin{bem} \qquad \par
%        Eine quadratische Matrix heißt genau dann orthogonal, wenn die Spaltenvektoren eine ON-System bilden.
%\end{bem}
%
%%----------------------------------------------------------------------------------------------------------
%%-----------------------------------------------Satz 4.18--------------------------------------------------
%%----------------------------------------------------------------------------------------------------------
%
%\begin{mysatz} \label{orthogonaleMatrix}  \qquad\par
%        Für Matrizen $A\in \R^{n \times n}$ sin folgende Aussagen äquivalent
%        \begin{itemize}
%                \item [1.] $A$ ist orthogonal
%                \item [2.] Die Abbildung $f_A:\R^n \rightarrow \R^n$ mit $f_a(v) = A\cdot v$ ist isometrisch
%                \item [3.] $A^T\cdot A = A\cdot A^T = E$
%                \item [4.] Wenn $A$ regulär, dann ist $A^{-1} = A^T$
%                \item [5.] Die Zeilenvektoren bilden ein ON-System
%        \end{itemize}
%\end{mysatz}
%
%\begin{bew}
...
%\begin{bem} \qquad \par
%        \begin{itemize}
%                \item [1.] Sei $A$ eine orthogonale Matrix. Dann ist $\det A=\pm1$, denn
%                                                         \begin{equation*}
%                                                                        1=\det E = \det A^T\cdot A = \det A^T \cdot \det A = (\det A)^2.
%                                                         \end{equation*}
%                \item [2.]Zwei Matrizen $A,B\in\R^{n \times n}$ heißen orthogonal ähnlich, wenn eine orthogonale Matrx $T$ existiert mit $T^{-1}AT=B$.
%        \end{itemize}
%\end{bem}
%
%
%%-----------------------------------------Beispiel-------------------------------------------------
%
%\begin{bsp} \textit{Beschreibung von orthogonalen Abbildungen in $\R^2$} \par
%        Sei $V=\R^2$ mit dem Standardskalarprodukt. Gesucht ist eine Matrix $A\in \R^{2\times 2}$ die orthogonal ist. Nach \ref{orthogonaleMatrix}$.3$ 
%        muss für $A$ gelten:
%        \begin{equation*}
...
%        \end{equation*}
%        Daher muss $a_{11}^2 + a_{12}^2=1 = a_{22}^2+a_{21}^2$ und $a_{11}a_{21} + a_{12}a_{22}=0$ gelten. Gleichzeitig müssen die Spalten der Matrix aber 
%        auch ein ON-System bilden. Daher gilt noch $a_{11}^2+a_{21}^2=1$ Für diese Gleichungen existiert ein $\varphi \in[0,2\pi]$ mit 
%        $a_{11}=\cos(\varphi), a_{21}=\sin(\varphi)$. Dann sind 
%        \begin{equation*}
%                D_{\varphi} = \begin{pmatrix} \cos(\varphi)&-\sin(\varphi)\\ \sin(\varphi)&\cos(\varphi) \end{pmatrix} \mbox{ oder }
%                S_{\varphi} = \begin{pmatrix} \cos(\varphi)&\sin(\varphi)\\ \sin(\varphi)&-\cos(\varphi) \end{pmatrix}
%        \end{equation*}
%        die beiden orthogonalen Matrizen in $\R^2$. Dabei hat $D_{\varphi}$ die Determinante $1$ und stellt eine Drehung um den Winkel $\varphi$ dar. Die 
%        Eigenwerte von $D_{\varphi}$ sind $\lambda_{1/2}=\cos(\varphi)\pm \sqrt{\cos(\varphi)^2-1}$. Diese sind demnach nur dann reell, wenn $\varphi$ den 
%        Wert $0$ oder $\pi$ annimmt. \par \medskip      
%        
%        Dagegen hat $S_{\varphi}$ die Determinante $-1$ und das charakteristische Polynom: 
%        charpol$S_{\varphi}=\lambda^2-\cos(\varphi)^2-\sin(\varphi)^2=\lambda^2-1$ mit den Eigenwerten $\pm 1$. $S_{\varphi}$ stellt eine Spiegelung der 
%        Richtung des Eigenvektors dar. 
%\end{bsp}
%
%
%%----------------------------------------Bemerkung-------------------------------------------------
...
%        Sei $f$ eine orthogonale Abbildung des $n$-dim. Vektorraums $V$ in sich. Dann existiert eine ON-Basis von $V$, bzgl der $f$ die Darstellungsmatrix
%                \begin{equation*}
%                        \begin{pmatrix}  1                        &               0                &              &                        & \cdots &\cdots &             &                        &      0
%                                        \\ 0                    &       \ddots &                &                        &                              &                       &               &                        &
%                                        \\                      &                                &      1 &                      &                              &                       &               &                                &
%                                        \\                        &                              &              & -1             &                              &                       &                 &                              &\vdots
%                                        \\ \vdots &                              &              &                        & \ddots &                     &               &                        &\vdots
...
%                                        \\                              &                                &              &                        &                              &                       &A_1  &                          &
%                                        \\                              &                                &              &                        &                              &                       &               & \ddots &      0 
%                                        \\       0              &                                &              & \cdots & \cdots &                     &               &    0   & A_1 \end{pmatrix}
%                \end{equation*}
%        besitzt.
%\end{mysatz}
%
